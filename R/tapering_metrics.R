...
#' }
stem_tapering_pc <- function(pc, slice_thickness = 0.1, maxtaperheight = 10,
                             interval = 1, buttress = FALSE,
                             thresholdbuttress = 0.0015, maxbuttressheight = 5,
                             plot = FALSE){
  if(buttress){
  ...
  x <- !(is.na(D) | is.na(S)) & (S > s) & (S < maxtaperheight) #& (D> Lower & D < Upper)
      ...
      if(nrow(T) > 1){
        if(plot == TRUE){
          ...
          taper_plot <- ggplot2::ggplot(T, ggplot2::aes(diameter, height)) +
            ...
            ggplot2::geom_smooth(method='lm') +
            ggplot2::geom_text(x = (max(T$diameter)+min(T$diameter))/2,
                               y = max(T$height)-0.5, label = lm_eqn(T),
                               parse = TRUE)
        } else {
          taper_plot <- NaN
        }
        ...
        rmse_d <- sqrt(mean((((S-b)/a)-D)^2, na.rm = TRUE))
      } else {
  ...
  return(list("a" = a, "b" = b, "r2" = r2, "rmse_d" = rmse_d, "rmse_c" = rmse_c,
              "T" = T, "plot" = taper_plot))
}
