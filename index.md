![](reference/figures/ITSMe_logo_RGB_nobg.jpg)
...
![](reference/figures/diameter_slice_example_butt.jpeg)
...
includes options for robust DBH estimation through the `how` argument
and returns DBH fit quality-control metrics such as `arc_coverage`and
`inner_circle_empty`. These metrics are also included in
[`summary_basic_pointcloud_metrics()`](https://lmterryn.github.io/ITSMe/reference/summary_basic_pointcloud_metrics.md)
when `"stem diameter"` is calculated.

See [`?dbh_pc`](https://lmterryn.github.io/ITSMe/reference/dbh_pc.md)
and
[`vignette("ITSMe")`](https://lmterryn.github.io/ITSMe/articles/ITSMe.md)
for details.
