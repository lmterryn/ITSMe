...
trunk) are removed. Down sampling of the point cloud is not required but can 
reduce computation time for very big tree point clouds (down sampling to 
2 cm is suggested).
...
out <- tree_height_pc(pc = tree_pc, plot = TRUE)
H <- out$h
plot_tree <- out$plotTree
```
...
out <- tree_height_pc(pc = tree_pc, dtm = DTM, r = 1.5, plot = TRUE)
H <- out$h
plot_tree <- out$plot
```
...
(and circle fitting) at 1.3 m with the `diameter_slice_pc` function.

```{r eval = FALSE}
# Measure DBH from the tree point cloud and plot the circle fitting
slice1 <- diameter_slice_pc(pc = tree_pc, slice_height = 1.3, 
                            slice_thickness = 0.06, plot = TRUE)
diameter <- slice1$diameter
...
through a default 6mm thick horizontal slice (from 1.27 m to 1.33 m above the 
lowest tree point). If branch or vegetation points are present in the slice, the 
lower (till 1.5 m above lowest point) trunk is extracted (so without 
branch/vegetation points) and the fit is done again. The `dbh_pc` function needs 
the tree point cloud as an input. If the argument *plot* is set TRUE it also 
plots the circle fitting. 
...
other parameters that can be optimised. The *thresholdR2* parameter 
(default = 0.001) determines when the trunk extraction is done. For point trees 
with non-circular stems this value should be increased. Also for smaller trees 
(DBH < 10 cm) or tree point cloud with a sub-optimal co-registration of scan 
positions, for example due to wind) a higher value will lead to better results. 
The *slice_thickness* parameter determines the thickness of the slice at 1.3 m 
and is default 6 mm. For tree point clouds with a lower point density at the 
stem (e.g. UAV-LS) increasing the slice_thickness to for example 20 cm is 
...
out <- classify_crown_pc(pc = tree_pc, plot = TRUE)
crown_pc <- out$crownpoints
plot <- out$plot
```
...
out <- classify_crown_pc(pc = tree_pc, minheight = 4, buttress = TRUE, 
                              plot = TRUE)
crown_pc <- out$crownpoints
plot <- out$plot
...
fitted to the (crown) points. The *concavity* (default=2) can be chosen and a 
...
Hs <- plot_tree_height_pcs(PCs_path = "path/to/point/clouds/folder/" , 
                         extension = ".ply",
...
crown_pcs <- plot_crown_classification_pcs(PCs_path = "path/to/point/clouds/folder/", 
                         extension = ".txt", 
                         OUT_path = "path/to/output/folder/", 
...
PAs <- plot_pa_pcs(PCs_path = "path/to/point/clouds/folder/" , 
                         extension = ".ply", 
...
# indicate crown = TRUE if you want to calculate projected area on the crown points only
# In this case: also specify the values for the crown classification chosen in previous step
AVs <- plot_av_pcs(PCs_path = "path/to/point/clouds/folder/" , 
                         extension = ".las", 
...
summary <- summary_basic_pointcloud_metrics(PCs_path = "path/to/point/clouds/folder_buttressed_trees/", 
...
summary_qsm_metrics(QSMs_path = "path/to/treeqsms/folder/", 
                    PCs_path = "path/to/point/clouds/folder/")
# Run the summary function with non-default settings with point cloud info
summary_qsm_metrics(QSMs_path = "path/to/treeqsms/folder/", version = "2.4.0", 
