...
The `how` argument in `dbh_pc()` controls how point-to-centre distances are summarised into
the final fitted radius. "mean" follows the original ITSMe logic, while "median" (new default) or adding a number makes the fit less sensitive to outlier points:

```{r dbh-how-example, eval=FALSE}
...
`dbh_pc()` returns quality-control metrics that can help identify unreliable
DBH fits:

- `arc_coverage`: proportion of angular sectors around the fitted DBH circle
  that contain nearby slice points;
- `inner_circle_empty`: whether the checked inner part of the fitted circle
  contains no slice points;


```{r dbh-qc-example, eval=FALSE}
...
Low `arc_coverage`, high `R2`, or `inner_circle_empty = FALSE` can be used to
flag DBH estimates for inspection.

When `"stem diameter"` is included in `summary_basic_pointcloud_metrics()`, the
DBH quality-control metrics are also included in the returned summary table:

```{r summary-dbh-qc-example, eval=FALSE}
...
For buttressed trees, `summary_basic_pointcloud_metrics()` may calculate
diameter above buttresses instead of DBH. In that case, the DBH-specific
quality-control columns are returned as `NA`.
